...
import {
  predictDurationFromML,
  predictDelayRiskFromML,
...
export const createServiceRequest = asyncHandler(async (req, res) => {
  ...
  const ruleBasedDuration = estimateDuration({
    serviceCategory: payload.serviceCategory,
    taskComplexity: payload.taskComplexity,
  });

  const estimatedDurationHours =
    payload.estimatedDurationHours || ruleBasedDuration.estimatedDurationHours;
  ...
  const delayRiskPayload = {
    ...durationPayload,
    gap_before_next_booking_mins: payload.gapBeforeNextBookingMins || 60,
    start_delay_mins: payload.startDelayMins || 0,
  };
  ...
  sendSuccess(
    res,
    {
      ...
      predictions: {
        ...
        delayRisk: delayRiskPrediction,
      },
    },
    "Service request created with ML predictions",
    201
  );
});
...
export const getServiceRequests = asyncHandler(async (req, res) => {
  ...
  sendSuccess(res, data, "Service requests retrieved successfully");
});
...
export const getServiceRequestById = asyncHandler(async (req, res) => {
  ...
  sendSuccess(res, data, "Service request retrieved successfully");
});