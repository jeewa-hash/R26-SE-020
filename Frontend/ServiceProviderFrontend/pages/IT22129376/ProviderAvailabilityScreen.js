...
export default function ProviderAvailabilityScreen({ navigation }) {
  ...
  return <SafeAreaView style={styles.container}>
    <View style={styles.header}><TouchableOpacity onPress={() => navigation.goBack()}><Ionicons name="arrow-back" size={24} color="#111827" /></TouchableOpacity><View style={styles.headerCopy}><Text style={styles.title}>Manage Availability</Text><Text style={styles.subtitle}>Set the time slots you are available for service bookings.</Text></View></View>
    <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
      <View style={styles.quickCard}><View><Text style={styles.sectionTitle}>Quick Availability Status</Text><Text style={[styles.quickStatus, { color: isAvailable ? '#047857' : '#B45309' }]}>{isAvailable ? 'Available' : 'Unavailable'}</Text></View><TouchableOpacity disabled={updatingStatus} style={[styles.statusButton, !isAvailable && styles.statusButtonAvailable]} onPress={toggleStatus}>{updatingStatus ? <ActivityIndicator color="#FFF" /> : <Text style={styles.statusButtonText}>{isAvailable ? 'Go Unavailable' : 'Go Available'}</Text>}</TouchableOpacity></View>
      <View style={styles.card}><Text style={styles.sectionTitle}>{editingId ? 'Edit Availability Slot' : 'Add Availability Slot'}</Text>
        <Text style={styles.label}>Date (YYYY-MM-DD)</Text><TextInput style={styles.input} value={form.date} onChangeText={(value) => setField('date', value)} placeholder="2026-09-01" />
        <View style={styles.row}><View style={styles.half}><Text style={styles.label}>Start (HH:mm)</Text><TextInput style={styles.input} value={form.startTime} onChangeText={(value) => setField('startTime', value)} placeholder="09:00" /></View><View style={styles.half}><Text style={styles.label}>End (HH:mm)</Text><TextInput style={styles.input} value={form.endTime} onChangeText={(value) => setField('endTime', value)} placeholder="12:00" /></View></View>
        <Text style={styles.label}>Notes (optional)</Text><TextInput style={styles.input} value={form.notes} onChangeText={(value) => setField('notes', value)} placeholder="Morning service hours" />
      ...
      <Text style={styles.listTitle}>Existing Availability</Text>
      {loading ? <View style={styles.state}><ActivityIndicator color="#667eea" /><Text style={styles.stateText}>Loading availability...</Text></View> : error ? <View style={styles.state}><Text style={styles.error}>{error}</Text><TouchableOpacity onPress={load}><Text style={styles.retry}>Try again</Text></TouchableOpacity></View> : slots.length === 0 ? <View style={styles.state}><Ionicons name="calendar-outline" size={30} color="#94A3B8" /><Text style={styles.stateText}>No availability slots added yet.</Text></View> : slots.map((slot) => <View key={slot._id} style={styles.slotCard}><View style={styles.slotTop}><View><Text style={styles.slotDate}>{slot.date}</Text><Text style={styles.slotTime}>{slot.startTime} – {slot.endTime}</Text></View><Text style={[styles.status, !slot.isAvailable && styles.unavailable]}>{slot.isAvailable ? 'Available' : 'Unavailable'}</Text></View>{slot.notes ? <Text style={styles.notes}>{slot.notes}</Text> : null}<View style={styles.slotActions}><TouchableOpacity onPress={() => edit(slot)}><Text style={styles.edit}>Edit</Text></TouchableOpacity><TouchableOpacity onPress={() => remove(slot)}><Text style={styles.delete}>Delete</Text></TouchableOpacity></View></View>)}
    </ScrollView>
  </SafeAreaView>;
}

const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: '#F8FAFC' }, header: { flexDirection: 'row', alignItems: 'center', gap: 14, padding: 18, backgroundColor: '#FFF', borderBottomWidth: 1, borderBottomColor: '#E5E7EB' }, headerCopy: { flex: 1 }, title: { fontSize: 21, fontWeight: '600', color: '#111827' }, subtitle: { color: '#64748B', fontSize: 12, marginTop: 2 }, content: { padding: 16, paddingBottom: 50 }, quickCard: { backgroundColor: '#FFF', borderRadius: 18, padding: 16, borderWidth: 1, borderColor: '#E5E7EB', marginBottom: 12, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 12 }, quickStatus: { fontSize: 13, fontWeight: '600' }, statusButton: { backgroundColor: '#B45309', borderRadius: 11, minWidth: 128, paddingHorizontal: 14, paddingVertical: 11, alignItems: 'center' }, statusButtonAvailable: { backgroundColor: '#059669' }, statusButtonText: { color: '#FFF', fontWeight: '600', fontSize: 12 }, card: { backgroundColor: '#FFF', borderRadius: 18, padding: 16, borderWidth: 1, borderColor: '#E5E7EB' }, sectionTitle: { fontSize: 17, fontWeight: '600', color: '#111827', marginBottom: 8 }, label: { color: '#475569', fontSize: 12, fontWeight: '500', marginTop: 10, marginBottom: 6 }, input: { borderWidth: 1, borderColor: '#CBD5E1', backgroundColor: '#F8FAFC', borderRadius: 11, paddingHorizontal: 12, minHeight: 46, color: '#111827' }, row: { flexDirection: 'row', gap: 10 }, half: { flex: 1 }, actions: { flexDirection: 'row', justifyContent: 'flex-end', gap: 10, marginTop: 16 }, saveButton: { backgroundColor: '#667eea', borderRadius: 11, paddingHorizontal: 16, paddingVertical: 12, minWidth: 145, alignItems: 'center' }, saveText: { color: '#FFF', fontWeight: '600' }, cancelButton: { paddingHorizontal: 16, paddingVertical: 12 }, cancelText: { color: '#64748B', fontWeight: '600' }, listTitle: { fontSize: 17, fontWeight: '600', color: '#111827', marginTop: 22, marginBottom: 10 }, state: { backgroundColor: '#FFF', borderRadius: 16, padding: 24, alignItems: 'center', gap: 9 }, stateText: { color: '#64748B' }, error: { color: '#B91C1C' }, retry: { color: '#667eea', fontWeight: '600' }, slotCard: { backgroundColor: '#FFF', borderRadius: 16, padding: 15, marginBottom: 10, borderWidth: 1, borderColor: '#E5E7EB' }, slotTop: { flexDirection: 'row', justifyContent: 'space-between' }, slotDate: { color: '#111827', fontWeight: '600' }, slotTime: { color: '#64748B', marginTop: 4 }, status: { color: '#047857', backgroundColor: '#D1FAE5', borderRadius: 10, paddingHorizontal: 9, paddingVertical: 5, fontSize: 11 }, unavailable: { color: '#B45309', backgroundColor: '#FEF3C7' }, notes: { color: '#64748B', marginTop: 9, fontSize: 12 }, slotActions: { flexDirection: 'row', justifyContent: 'flex-end', gap: 20, marginTop: 12 }, edit: { color: '#4F46E5', fontWeight: '600' }, delete: { color: '#DC2626', fontWeight: '600' },
});
