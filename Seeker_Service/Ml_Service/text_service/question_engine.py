...
class QuestionEngine:
    ...
    # -------------------------
    # Normalize flow keys (safely handles None)
    # -------------------------
        ...
        while True:
            ...
            if answer_key in answers:
                ...
                if isinstance(next_q, dict):
                    saved = self._normalize_answer(answers[answer_key])
                    current_step = next_q.get(saved, next_q.get("default"))
                else:
                    current_step = next_q
    ...
    def start(self, service, sub_service=None, extracted=None):
        ...
        # --- Get flow for this service; if missing, use fallback ---
        flow_dict = TEXT_QUESTION_FLOW.get(service)
        if flow_dict is None:
            print(f"⚠️ Unknown service: '{service}'. Using fallback flow.")
            flow_dict = {
                1: {"question": "What service do you need?", "answer_key": "service_type"},
                2: {"question": "Where is the service located?", "answer_key": "location"}
            }

        flow = self._normalize_flow(flow_dict)
        start_step = 1
        extracted = extracted or {}

        # --- smart routing by sub_service (unchanged) ---
        if sub_service:
    ...
    def get_current_question(self, session_id):
        ...
        if not session:
            return None
        flow_dict = TEXT_QUESTION_FLOW.get(session["service"])
        if flow_dict is None:
            return None
        flow = self._normalize_flow(flow_dict)
